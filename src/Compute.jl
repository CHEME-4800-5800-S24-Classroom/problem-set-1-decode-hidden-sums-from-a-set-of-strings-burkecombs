...
function _decode_part_1(model::MyPuzzleRecordModel)::Int64
    ...
    # with this line, we retrieve the characters -
    ...
    # now, we join the characters and parse the value
    return value |> join |> x-> parse(Int64, x);
end

function _decode_part_2(model::MyPuzzleRecordModel)::Int64
    
    # now, we need to search for one, two, ..., nine, and replace these with 1,2,...,9
    # Then we can parse the value using _decode_part_1
    ...
    # now, we use _decode_part_1 to parse the value
